...
func (r RedisCommonBehaviour[T]) Get(ctx context.Context, id entity.ID) (T, error) {
	...
	if err != nil {
		...
		logrus.WithError(err).WithField("id", id).Error("failed to retrieve from redis")
		return t, err
	}
...
func (r RedisCommonBehaviour[T]) Save(ctx context.Context, t T) error {
	cmd := r.client.B().
		JsonSet().Key(t.EntityID().String()).Path("$").Value(string(jsonhelper.Encode(t))).Build()
	if err := r.client.Do(ctx, cmd).Error(); err != nil {
		logrus.WithError(err).WithField("entity", t).Error("failed to save to redis")
		return err
	}
	return nil
}
